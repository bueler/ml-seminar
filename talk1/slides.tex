...
\item possibilities:
    ...
    \end{itemize}
\item $\sigma$ is usually(?) increasing, with bounded derivative
\end{itemize}
\end{frame}
...
\item a \alert{trained neuron} is one with fixed parameters, so
    $$a = \sigma(v)$$
is a function $a:\RR^n \to \RR^1$
\item we might write when training:
    $$a = \sigma(v;w,b)$$
\end{itemize}
}
\end{column}
...
\begin{itemize}
\item $w^\top$ is a row vector \dots one for each neuron
\item many neurons gives matrix multiplication:
    $$W = \begin{bmatrix} w_{11}
    \end{bmatrix}$$
\item $\sigma$ applied entrywise
\item FIXME
\end{itemize}
\end{frame}
