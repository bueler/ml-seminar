...
\item Rosenblatt (1958): from biological motivation, proposes a \alert{perceptron}, a single artificial neuron with binary output:
    ...
    \end{itemize}
\item Minsky \& Papert (1969): a single layer of perceptrons cannot learn the XOR function
    \begin{itemize}
    \item[$\circ$] single layer perceptrons are \alert{linear separators}
    \item[$\circ$] \emph{support vector machines} are \emph{perceptrons of optimal stability}
    \end{itemize}
\item feedforward artificial neural networks (ANN; next!) are sometimes called \emph{multilayer perceptrons}
    \begin{itemize}
