...
\item let's give all the parameters a single-letter name:
   $$p = \{W^{[2]},W^{[3]},\dots,W^{[L]},b^{[2]},b^{[3]},\dots,b^{[L]}\}$$
    ...
    \end{itemize}
\item define the cost (misfit) of the network for a single labeled data pair:
    $$C^{\{i\}}(p) = \frac{1}{2} \left\|y^{\{i\}} - a^{[L]}(x^{\{i\}})\right\|_2^2$$

    \begin{itemize}
    \item[$\circ$] $1/2$ is a convenience constant
    \end{itemize}
\item let $C(p)$ be the total \alert{cost functional}, or \alert{objective}, namely the \alert{average} misfit over all the labeled data:
    $$C(p) = \frac{1}{N} \sum_{i=1}^N C^{\{i\}}(p) = \frac{1}{2N} \sum_{i=1}^N \left\|y^{\{i\}} - a^{[L]}(x^{\{i\}})\right\|_2^2$$
\end{itemize}
\end{frame}
...
\vspace{-2mm}
\begin{center}
\includegraphics[width=0.95\textwidth]{figs/nonlinearleastsquares}
\end{center}

\medskip
\item training a neural net is \alert{nonlinear least-squares optimization}
    \begin{itemize}
    \item[$\circ$] $y^{\{i\}} - a^{[L]}(x^{\{i\}})$ is the \alert{residual vector} for $i$th data
...
\begin{itemize}
\item \alert{deep learning} if $L>2$
\end{itemize}
\end{frame}
